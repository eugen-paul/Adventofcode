...
public class Day9 extends SolutionTemplate {
    ...
    public long doPuzzle2tt(List<String> eventData) {
        long response = 0;
        ...
        for (int i = 0; i < m.size(); i++) {
            ...
            for (int k = i + 1; k < m.size(); k++) {
                ...
                int fx = min(a.getX(), b.getX());
                int tx = max(a.getX(), b.getX());
                int fy = min(a.getY(), b.getY());
                int ty = max(a.getY(), b.getY());

                boolean ok = true;
                for (int x = fx; x <= tx && ok; x++) {
                    ok = ok && isIn(hLines, vLines, x, fy);
                    ok = ok && isIn(hLines, vLines, x, ty);
                }
                for (int y = fy; y <= ty && ok; y++) {
                    ok = ok && isIn(hLines, vLines, fx, y);
                    ok = ok && isIn(hLines, vLines, tx, y);
                }
